...
def load_data():
    ...
    try:
        ...
        for index in tqdm(range(start_index, end_index)):
            ...
            tmp = [str(item.start), str(item.end)]
            # 国家信息
            if item.country_code in ('CN', 'HK', 'MO', 'TW'):
                tmp.append('CN')
                tmp.append('中国')
            else:
                tmp.append(item.country_code)
                tmp.append(country_translate_dict.get(item.country, item.country))
            # 区域信息
            if item.country_code == 'TW':
                tmp.append('台湾')
            else:
                tmp.append(region_translate_dict.get(item.region, item.region))
            # 城市信息
            if item.country_code == 'HK':
                # 香港数据精细到了街道，这里直接统一香港
                tmp.append('香港')
            else:
        ...
        # 避免每次变更都写数据拖慢处理进度
        with open('./ip_data.txt', 'w', encoding='utf-8') as f:
